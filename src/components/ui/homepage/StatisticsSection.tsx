...
export function StatisticsSection({ stats, loading }: StatisticsSectionProps) {
  ...
  const displayBlocks =
    loading || error
      ? [
          { icon: <Users className="w-full h-full" style={{ color: '#D8DAE4' }} />, number: '—', label: 'Members' },
          { icon: <ImageIcon className="w-full h-full" style={{ color: '#D8DAE4' }} />, number: '—', label: 'Photos' },
          { icon: <Calendar className="w-full h-full" style={{ color: '#D8DAE4' }} />, number: '—', label: 'Events' },
          { icon: <Award className="w-full h-full" style={{ color: '#D8DAE4' }} />, number: '—', label: 'Awards' },
        ]
      : statBlocks
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 sm:flex sm:flex-row flex-wrap justify-center sm:justify-between items-center gap-4 sm:gap-6 md:gap-8 lg:gap-12">
        {displayBlocks.map((stat, index) => (
          <StatBlock key={index} icon={stat.icon} number={stat.number} label={stat.label} />
        ))}
      </div>
    </section>
  )
