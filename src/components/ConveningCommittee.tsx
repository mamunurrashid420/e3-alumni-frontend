...
export function ConveningCommittee() {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="bg-primary-custom text-white px-6 py-4">
            <h1 className="text-2xl sm:text-3xl font-bold">Convening Committee</h1>
          ...
          <div className="p-4 sm:p-6 overflow-x-auto">
            ...
            {!loading && !error && items.length === 0 && (
              <p className="text-center py-8 text-gray-500">No members to display.</p>
            )}
            {!loading && !error && items.length > 0 && (
                ...
                {items.map((row) => (
                  ...
                  >
                    ...
                    <div className="px-4 pb-4 text-center space-y-1 text-black">
                      ...
                      {row.designation && (
                        <p className="text-sm font-medium text-black">{row.designation}</p>
                      )}
                      {row.occupation && (
                        <p className="text-sm text-black">{row.occupation}</p>
                      )}
                      {row.mobile_number && (
